    ...
    <div class="grid grid-cols-1 xl:grid-cols-3 gap-4">
        <div class="xl:col-span-2 bg-slate-900 border border-slate-800 rounded-xl p-5">
            ...
            <div class="mt-4 flex gap-3 flex-wrap items-center">
                {{-- Ranking --}}
                <button type="button" onclick="openRankingModal()"
...
{{-- ===== MODAL DE RANKING ===== --}}
<div id="ranking-modal" class="fixed inset-0 z-50 hidden">
    <div class="absolute inset-0 bg-black/70" onclick="closeRankingModal()"></div>
    <div class="relative max-w-sm mx-auto mt-24 bg-slate-900 border border-slate-800 rounded-xl p-5">
        <div class="flex items-center justify-between mb-4">
            <h3 class="text-white font-semibold">Top Ranking</h3>
            <div class="flex items-center gap-3">
                <a href="{{ route('rankings.show', $group) }}" class="text-xs text-emerald-400 hover:text-emerald-300 transition-colors">Ver completo →</a>
                <button class="text-slate-400 hover:text-white text-sm" onclick="closeRankingModal()">Fechar</button>
            </div>
        </div>
        @if($rankings->isEmpty())
        <div class="flex flex-col items-center justify-center py-8 text-slate-500">
            <span class="text-3xl mb-2">🏆</span>
            <p class="text-sm">Ainda sem ranking calculado.</p>
        </div>
        @else
        <div class="space-y-2 max-h-80 overflow-y-auto pr-1">
            @foreach($rankings->take(10) as $ranking)
            <div class="flex items-center justify-between text-sm bg-slate-800/70 rounded-lg px-3 py-2.5">
                <div class="flex items-center gap-2.5">
                    <span class="text-xs font-bold w-5 text-center {{ $ranking->position === 1 ? 'text-amber-400' : ($ranking->position === 2 ? 'text-slate-300' : ($ranking->position === 3 ? 'text-amber-600' : 'text-slate-500')) }}">
                        #{{ $ranking->position }}
                    </span>
                    <span class="text-slate-200">{{ $ranking->user->name }}</span>
                </div>
...
<div id="members-modal" class="fixed inset-0 z-50 hidden">
    <div class="absolute inset-0 bg-black/70" onclick="closeMembersModal()"></div>
    <div class="relative max-w-sm mx-auto mt-24 bg-slate-900 border border-slate-800 rounded-xl p-5">
        <div class="flex items-center justify-between mb-4">
            <h3 class="text-white font-semibold">Membros ({{ $group->members->count() }})</h3>
            <button class="text-slate-400 hover:text-white text-sm" onclick="closeMembersModal()">Fechar</button>
        </div>
        <div class="space-y-2 max-h-80 overflow-y-auto pr-1">
            @foreach($group->members as $member)
            <div class="flex items-center justify-between gap-3 text-sm bg-slate-800/60 rounded-lg px-3 py-2">
                <div class="flex items-center gap-2.5 min-w-0">
                    <div class="w-8 h-8 rounded-full bg-gradient-to-br from-emerald-500 to-blue-600 flex items-center justify-center text-xs font-bold text-white shrink-0">
                        {{ strtoupper(substr($member->name, 0, 1)) }}
                    </div>
                    <div class="min-w-0">
                        <p class="text-slate-200 truncate">{{ $member->name }}</p>
                        <span class="text-xs {{ $member->pivot->role === 'admin' ? 'text-amber-400' : 'text-slate-500' }}">
                            {{ $member->pivot->role === 'admin' ? 'Admin' : 'Jogador' }}
                        </span>
                    </div>
                </div>
                @if($member->pivot->presence_confirmed)
                <span class="text-emerald-400 shrink-0" title="Presença confirmada">
                    <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
                    </svg>
                </span>
